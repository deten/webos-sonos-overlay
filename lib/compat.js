...
var TESTED = [
  ...
  { release: '6.5.0', model: 'OLED55C14LB', note: 'LG C1, reported by a tester' }
];
...
// ---------------------------------------------------------------------------
// /run is a tmpfs and the nyx json files are written during boot. The boot hook
// starts this service early enough that they can still be missing, which is how a
// report came back with "unknown model". nyx-cmd queries the same data directly
// and does not depend on those files existing yet.
function nyxCmd(group, field) {
  try {
    var out = cp.execSync('/usr/bin/nyx-cmd ' + group + ' query ' + field + ' 2>/dev/null',
...
function readPlatform() {
  ...
  if (!info.release) {
    var nyxRel = nyxCmd('OSInfo', 'webos_release');
    if (nyxRel) { info.release = nyxRel; info.source = 'nyx-cmd'; }
  }
  ...
  if (!info.model) info.model = nyxCmd('DeviceInfo', 'product_id');
  if (!info.board) info.board = nyxCmd('DeviceInfo', 'board_type');

  return info;
}

// True when a re-read might still turn something up, so callers can refresh
// rather than reporting "unknown" for the life of the process.
function isIncomplete(info) {
  return !info || !info.release || !info.model;
}
