...
function openInputDeviceViaChild(devicePath, onEvent, onError) {
  ...
  // everEmitted is what tells a live device apart from one that merely opened,
  // which is the distinction a diagnostics report needs.
  handle = {
    path:   devicePath,
    close:  close,
    isLive: function() { return everEmitted; }
  };
  return handle;
}
...
function openInputDevicesMulti(devicePaths, onEvent, onError) {
  ...
  return {
    ...
    live: function() {
      return handles.filter(function(h) { return h.isLive && h.isLive(); })
